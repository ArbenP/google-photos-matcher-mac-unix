...
def searchMedia(path, title, mediaMoved, nonEdited, editedWord):
    ...
    realTitle = str(
        title.rsplit(".", 1)[0] + "-" + editedWord + "." + title.rsplit(".", 1)[1]
    )
    movedTitle = None
    movedFilePath = None
    filepath = (
        path + "/" + realTitle
    )  # First we check if exists an edited version of the image
    if not os.path.exists(filepath):
        ...
        if (
            ...
            or len(glob.glob(path + "/" + title + "*(1).json")) > 0
        ):
            realTitle = title
            filepath = (
                path + "/" + realTitle
            )  # If not, check if exists the path with the same name
            if not os.path.exists(filepath):
                realTitle = checkIfSameName(
                    title, title, mediaMoved, 1
                )  # If not, check if exists the path to the same name adding (1), (2), etc
                filepath = str(path + "/" + realTitle)
                if not os.path.exists(filepath):
                    title = (
                        (title.rsplit(".", 1)[0])[:47] + "." + title.rsplit(".", 1)[1]
                    )  # Sometimes title is limited to 47 characters, check also that
                    realTitle = str(
                        title.rsplit(".", 1)[0]
                        + "-"
                        + editedWord
                    ...
                    if not os.path.exists(filepath):
                        realTitle = str(
                            title.rsplit(".", 1)[0] + "(1)." + title.rsplit(".", 1)[1]
                        )
                        filepath = path + "/" + realTitle
                        if (
                            not os.path.exists(filepath)
                            or len(glob.glob(path + "/" + title + "*(1).json")) > 0
                        ):  # os.path.exists(path + "/" + title + "(1).json"):
                            realTitle = title
                            filepath = path + "/" + realTitle
                            if not os.path.exists(filepath):
...
def set_Images_EXIF(filepath, lat, lng, altitude, timeStamp):
    ...
    try:
        ...
        if is_New_GPS_Valid(
            altitude, lat, lng
        ) and is_Existing_Image_GPS_Tag_Inconsistent(exif_dict["GPS"]):
            ...
            if abs(altitude) > 0 or (
                ...
                and not has_Property_Dict(gps_ifd, piexif.GPSIFD.GPSLongitude)
            ):
                if not has_Property_Dict(
                    gps_ifd, piexif.GPSIFD.GPSAltitudeRef
                ) or not has_Property_Dict(gps_ifd, piexif.GPSIFD.GPSAltitude):
            ...
            ) or not has_Property_Dict(gps_ifd, piexif.GPSIFD.GPSLatitude):
            ...
            ) or not has_Property_Dict(gps_ifd, piexif.GPSIFD.GPSLongitude):
...
def set_Image_EXIF_ExifTool(filepath, lat, lng, altitude, timeStamp):
    """Set EXIF metadata for HEIC, PNG, and other formats using exiftool"""
    try:
        ...
        if is_New_GPS_Valid(altitude, lat, lng):
            tags["EXIF:GPSLatitude"] = abs(lat)
            tags["EXIF:GPSLatitudeRef"] = "N" if lat >= 0 else "S"
            tags["EXIF:GPSLongitude"] = abs(lng)
            tags["EXIF:GPSLongitudeRef"] = "E" if lng >= 0 else "W"
            tags["EXIF:GPSAltitude"] = abs(altitude)
            tags["EXIF:GPSAltitudeRef"] = (
                "Above Sea Level" if altitude >= 0 else "Below Sea Level"
            )

        et.set_tags([filepath], tags=tags, params=["-overwrite_original"])
        print("Image Exif updated (exiftool)")
    except Exception as e:
        print("Warning: Could not set EXIF for " + filepath + ": " + str(e))


def set_QuickTime_Video_EXIF(filepath, lat, lng, altitude, timeStamp=None):
    try:
        updates = []

        # Update creation date if timestamp provided
        if timeStamp:
            dateTime = datetime.fromtimestamp(timeStamp).strftime("%Y:%m:%d %H:%M:%S")
            et.set_tags(
                [filepath],
        ...
        if is_New_GPS_Valid(
            altitude, lat, lng
        ) and is_Existing_Video_GPS_Tag_Inconsistent(videoMetadata):
...
def get_Video_GPS_Tag(filepath, tag):
    return et.get_tags(
        [filepath], tags={"QuickTime:" + tag + ":GPSCoordinates"}, params=["-g0:1"]
    )
...
def set_Video_GPS_Tag(filepath, tag, lat, lng, altitude):
    et.set_tags(
        [filepath],
        tags={
            "QuickTime:" + tag + ":GPSCoordinates": str(lat)
            + " "
            + str(lng)
            + " "
            + str(altitude)
...
def is_Existing_Image_GPS_Tag_Inconsistent(gpsDict):
    return (
        gpsDict is None
        or len(gpsDict) == 0
        or not piexif.GPSIFD.GPSAltitudeRef in gpsDict
        or gpsDict[piexif.GPSIFD.GPSAltitudeRef] is None
        or not piexif.GPSIFD.GPSLatitudeRef in gpsDict
        or gpsDict[piexif.GPSIFD.GPSLatitudeRef] is None
        or not piexif.GPSIFD.GPSLongitudeRef in gpsDict
        or gpsDict[piexif.GPSIFD.GPSLongitudeRef] is None
        or not piexif.GPSIFD.GPSAltitude in gpsDict
        or gpsDict[piexif.GPSIFD.GPSAltitude] is None
        or len(gpsDict[piexif.GPSIFD.GPSAltitude]) == 0
        or not piexif.GPSIFD.GPSLatitude in gpsDict
        or gpsDict[piexif.GPSIFD.GPSLatitude] is None
        or len(gpsDict[piexif.GPSIFD.GPSLatitude]) == 0
        or not piexif.GPSIFD.GPSLongitude in gpsDict
        or gpsDict[piexif.GPSIFD.GPSLongitude] is None
        or len(gpsDict[piexif.GPSIFD.GPSLongitude]) == 0
    )
...
def is_Existing_Video_GPS_Tag_Inconsistent(videoExifDict):
    return not (
        videoExifDict
        and len(videoExifDict) > 0
        and "QuickTime:GPSCoordinates" in videoExifDict[0]
        and videoExifDict[0]["QuickTime:GPSCoordinates"]
    )
